...
class RINGexperiment(object):
    ...
    def setCorrType(self, corrtype, verbal=False):
        ...
        if corrtype == 'chi':
            ...
            if verbal: print("Using MI correlation metric")#Modif AF, remove normalized
            ...
            self.correlationfunc = self._ratioMutualinformation#Modif AF , add new metric as the ratio of MI mutated over MI bg
            if verbal: print("Using ratio Mutual information metric")
    ...
    def _ratiomutualinformation(self, n, b,c,d,nb,bb,cb,db):
        return float(self._mutualinformation(n,b,c,d)/self._mutualinformation(nb,bb,cb,db))
    ...
    def GetCount(self,i,j, prefix):
        ...
        a = n-b-c-d
        if n>0:
            return (a,c,b,d,n,d/float(n),a/float(n))
        else:
            return (a,c,b,d,n,0,0)
    

    def DisplayFreq(self):
        seqlen = self.getMaxArrayIndex()
        UU=np.zeros((seqlen,seqlen))
        #for prefix in ('bg','ex'):  
        for prefix in ('bg','ex'): 
            #print prefix 
            with open(args.outputFile+"FREQUENCY_COUNT"+prefix+".txt",'w') as OUT:
                OUT.write("i-1based \t j-1based \t UU \t UM \t MU \t MM \t #Reads \t MM/#Reads  \t UU/#Reads \n" )
                for i in range(seqlen):
                    for j in range(i, seqlen):
                        #print i+1,j+1, self.GetCount(i,j, prefix)
                        OUT.write("%i \t %i \t %i \t %i \t %i \t %i \t %i \t %.6f \t %.2f \n"%(i+1,j+1,self.GetCount(i,j, prefix)[0], self.GetCount(i,j, prefix)[1],self.GetCount(i,j, prefix)[2],self.GetCount(i,j, prefix)[3],self.GetCount(i,j, prefix)[4] ,self.GetCount(i,j, prefix)[5] ,self.GetCount(i,j, prefix)[6]))
                        UU[i][j]=self.GetCount(i,j, prefix)[0]

        '''
        mask =np.zeros_like(UU, dtype=np.bool)
        mask[np.triu_indices_from(mask)]=True

        fig,ax= plt.subplots(figsize=(seqlen,seqlen))
        im=ax.imshow(UU)
        #heatmap=sns.heatmap(corr_matrix, mask=mask, square=True, linewidths=.5, cmap='coolwarm',cbar_kws={'shrink':.4,'ticks':[0,500,1000,4000]},vmin=0, vmax=10000,annot=True,annot_kws={"size":12})
        ax.set_yticklabels([i+1 for i in range (seqlen) ],rotation=0)
        ax.set_xticklabels([i+1 for i in range (seqlen) ])
        fig.tight_layout()
        plt.show()
        #sns.set_style({'xtick.bottom':True},{'ytick.left':True} )
        #heatmap.get_figure().savefig('UU.png',bbox_inches='tight')
    ...
    def _correlationMatrix(self, prefix, corrbuffer, mindepth, mincount):
        ...
        read_bg = getattr(self, 'bg_readarr')
        inotj_bg = getattr(self, 'bg_inotjarr')
        comut_bg = getattr(self, 'bg_comutarr')
